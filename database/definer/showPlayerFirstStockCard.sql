CREATE PROCEDURE showPlayerFirstStockCard(
    IN user_login_param VARCHAR(30),
    IN user_password_param VARCHAR(255),
    IN game_id_param INT
)
COMMENT "(user_login VARCHAR(30), user_password VARCHAR(255), game_id_param INT) - Показывает первую карту в запасе игрока."
...
BEGIN
    ...
    IF is_valid_password THEN
        SELECT 'Stock' AS CardSource, Cards.ID, Cards.card_value, Cards.card_color, Cards.game_id
        FROM Cards
        WHERE ID = (SELECT first_card_stock FROM Players WHERE ID = player_id_val);
    ELSE
